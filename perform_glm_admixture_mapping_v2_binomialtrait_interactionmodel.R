...
if(length(args)<5){
stop("usage is Rscript perform_glm_admixture_mapping_v2_binomialtrait_interactionmodel.R genotypes_file hybrid_index_file phenotypes_file focal_column_number interaction_marker")
}

infile <- as.character(args[1])
out<-paste(infile,"_results_v2_binomial_interaction_model",sep="")
...
p<-t(p)
results<-cbind(names[x],p,(summary(model1)$coef[,"z value"])[4],like_diff)
if(track==0){
write.table(results,file=out,append=TRUE,col.names=c("chrom.marker","intercept","mixture_prop","site1","site2","interaction","z-value","likelihood-diff"),row.names=F,sep="\t",quote=FALSE)
track=1
} else{
write.table(cbind(names[x],p,(summary(model1)$coef[,"z value"])[4],like_diff),file=out,append=TRUE,col.names=FALSE,row.names=F,sep="\t",quote=FALSE)
