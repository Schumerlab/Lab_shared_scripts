######perform admixture mapping permutations given a genotypes file, a file with hybrid index (expects hybrid index to be in the second column, a phenotypes file, and a column number for the phenotype of interest
...
if(track==0){
write.table(results,file=out,append=TRUE,col.names=c("chrom.marker","intercept","mixture_prop","site","t-value","likelihood-diff"),row.names=F,sep="\t",quote=FALSE)
track=1
} else{
