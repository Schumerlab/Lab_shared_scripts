...
if(length(arrArgs)<3){
stop("Rscript genotypes_to_ancestry_plot.R genos_file chromname idlist_with_header\n");
}
options(scipen=999)
...
pdf(paste(focalid,"_",chr,".pdf",sep=""),width=5.5,height=3)
plot(genos[,1]/1e6,genos[,2],type="l",lwd=2,col="blue",ylim=c(0,2),xlab="Position (Mb)",ylab="Number of parent2 alleles")
dev.off()
