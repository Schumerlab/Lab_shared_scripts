...
#revision notes: add indel realignment 11/8/21 MES
#as written will only work on sherlock because of module load!
...
system("java -jar /home/groups/schumer/shared_bin/CreateSequenceDictionary.jar R\=$genome1 O\=$dict");
...
while(my $line1 =<IN1>){
	...
	my $read2=$read_array[1]; chomp $read2;
     ...
     system("samtools index $sorted1");
     my $dedup1="$sorted1".".dedup";
     my $metrics1="$read1".".metrics";

     my $unique1 = "$sorted1".".unique.bam";
     $unique1 =~ s/sorted.bam.unique/sorted.unique/g;
     system("samtools view -b -q 30 $sorted1 > $unique1");

	system("java -jar /home/groups/schumer/shared_bin/BuildBamIndex.jar INPUT\=$unique1");
	my $intervals="$read1".".intervals";

	system("java -jar /home/groups/schumer/shared_bin/GenomeAnalysisTK.jar -T RealignerTargetCreator -R $genome1 -I $unique1 -o $intervals");

	print "java -jar /home/groups/schumer/shared_bin/GenomeAnalysisTK.jar -T RealignerTargetCreator -R $genome1 -I $unique1 -o $intervals\n";

	my $realigned = "$sorted1".".unique.realigned.bam";
	system("java -jar /home/groups/schumer/shared_bin/GenomeAnalysisTK.jar -T IndelRealigner -R $genome1 -I $unique1 -targetIntervals $intervals -o $realigned");

     system("samtools index $realigned");
